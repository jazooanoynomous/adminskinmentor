...
module.exports = class UserControl {
    ...
    static async doctorRegister(req, res) {
        const { name, email, password, specialty, cliniAddress, phoneNumber } = req.body;
        try {
            // const user = await User.findOne({ email });
            // if (user) {
            //     return res.status(422).json({ error: "User already with this email" });
            // }
            const user = await User.findOne({ email });
    ...
    static async doctorLogin (req,res) {
        ...
        if(doMatch){
            ...
            res.json({token, userRole:user.role, userId:user._id, userName:user.name, 
...
static async updateDoctorAvailability(req, res) {
    ...
    const { startAvailability, endAvailability } = req.body; // New start and end availability times
    console.log('this id at backend ',idDoctor )

    console.log('these are time at backend ',startAvailability, endAvailability)
    try {
  ...
  static async getDoctorsBySpecialty(req, res) {
    ...
    static async deleteDoctorById(req, res) {
    try {
      const Docid = await User.findByIdAndDelete(req.params.id);
      if (!Docid) {
